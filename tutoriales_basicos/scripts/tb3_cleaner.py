...
class RobotCleaner:
    ...
    def __init__(self):
      self.pose_subscriber=rospy.Subscriber("/tb0/amcl_pose", PoseWithCovarianceStamped,self.poseCallback,queue_size=1)
      self.velocity_publisher=rospy.Publisher('/tb0/cmd_vel', Twist, queue_size=10)
    ...
    def poseCallback(self,data):
        #print('Recibido')
        self.turtlesim_pose.x=data.pose.pose.position.x
        self.turtlesim_pose.y=data.pose.pose.position.y
        orientation_q = data.pose.pose.orientation
        orientation_list = [orientation_q.x, orientation_q.y, orientation_q.z, orientation_q.w]
        (roll, pitch, yaw) = euler_from_quaternion (orientation_list)
        self.turtlesim_pose.theta=yaw
        self.poseflag=True
    ...
    def moveGoal(self,goal_pose,distance_tolerance):
        ...
        while True:
              ...
              ka=1.5#1.5
              xvel=0.05
              #xvel=kl*self.getDistance2(self.turtlesim_pose,goal_pose)
              if(xvel>0.22):
                  xvel=0.22
    ...
    def spiralClean(self):
        ...
        while True:
              ...
              self.velocity_publisher.publish(self.vel_msg)
              self.rate.sleep()
              if not((self.turtlesim_pose.x<0.7)and(self.turtlesim_pose.x>-0.7)and(self.turtlesim_pose.y<0.7)and(self.turtlesim_pose.y>-0.7)):
                   break
        #make zero the linear and angular velocity
        self.vel_msg.linear.x=0
        self.vel_msg.angular.z=0
        ...
        if option==1:
           ...
           RC.setDesiredOrientation2(float(int(sys.argv[2])))
            ...
            while(cont):
              ...
              if (x>-1.2 and x<1.2)and(y>-1.2 and y<1.2):
                  goal_pose=Pose()
                  goal_pose.x=x
                  goal_pose.y=y
                  goal_pose.theta=0
                  RC.moveGoal(goal_pose,0.02)
              else:
